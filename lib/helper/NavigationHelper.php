...
function buildNavigationPath($subject)
{
    ...
    switch(get_class($subject)) {
        case 'SfGuardUserProfile' :
            ...
            $slugs[] = link_to(format_number_choice('[0] No Boards|[1]%count% Board|(1,+Inf]%count% Boards', array('%count%' => $boards_count), $boards_count), array('sf_route' => 'user', 'nick' => $subject->getNick()));
        ...
        case 'Scene' :
            ...
            $slugs[] = link_to($subject->getSceneTime()->getClip()->getName(), array('sf_route' => 'scene', 'username_slug' => $subject->getSfGuardUserProfile()->getNick(), 'id' => $subject->getId(), 'board_id' => $subject->getBoardId()));
