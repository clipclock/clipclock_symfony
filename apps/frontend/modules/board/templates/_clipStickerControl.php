...
<?php foreach($scene_times as $key => $scene_time):?>
	<li<?php if($tabs_to_hide):?> style="display: none"<?php endif;?> id='sticker_<?php echo $reclip_id?>_<?php echo $scene_time['id']?>' class="sticker-tab<?php echo $key == $active_key ? ' active' : '' ?>">
		<a href="<?php echo url_for('scene', array('board_id' => $scene_time['board_id'], 'id' => $scene_time['id'], 'username_slug' => $scene_time['nick'])) ?>" data-url="<?php echo url_for('@sticker_scene_change?scene_id=' . $scene_time['id']) ?>" data-reclip-id="<?php echo $reclip_id ?>" data-scene-id="<?php echo $scene_time['id'] ?>">
