function cuSel(params){
    jQuery(document).ready(function(){
        ...
        {
                var chEl=jQuery(this),chElWid=chEl.outerWidth(),chElClass=chEl.attr("class"),chElId=chEl.attr("id"),chElName=chEl.attr("name"),defaultVal=chEl.val(),activeOpt=chEl.find("option:selected").eq(0),defaultText=activeOpt.text(),disabledSel=chEl.attr("disabled"),scrollArrows=params.scrollArrows,chElOnChange=chEl.attr("onchange"),chElTab=chEl.attr("tabindex");                
                ...
                spanStr+'</div><div class="cusel-bottom-border"></div></div>'+'<input type="hidden" id="'+chElId+'" name="'+chElName+'" value="'+defaultVal+'" />'+'</div>';
                chEl.replaceWith(cuselFrame);                
                if(chElOnChange)jQuery("#"+chElId).bind('change',chElOnChange);
                var newSel=jQuery("#cuselFrame-"+chElId),arrSpan=newSel.find("span"),defaultHeight;                
                if (params.spanHeight != null) {
                    defaultHeight = params.spanHeight;
                } else {
...

        {
...

                {
                    ...
                    {
                        jQuery(".cusel-scroll-wrap").css("display","none").closest('.cusel').removeClass('cuselFocus');
                        cuselWrap.css("display","block").closest('.cusel').addClass('cuselFocus');
                    ...
                    {
                        cuselWrap.css("display","none").closest('.cusel').removeClass('cuselFocus');
                    }
                }
                ...
                else if(clicked.is("span")&&clicked.parents(".cusel-scroll-wrap").is("div"))
                {
                    ...
                    clicked.parents(".cusel-scroll-wrap").find(".cuselActive").eq(0).removeClass("cuselActive").end().parents(".cusel-scroll-wrap").next().val(clickedVal).end().prev().text(clicked.text()).end().css("display","none").closest('.cusel').removeClass('cuselFocus');
...

        {
...

                {
...

                    {
                        ...
                        {
                            var idScrollWrap=scrollWrap.attr("id"),hOption=scrollWrap.find("span").eq(0).outerHeight();
                ...
                if(key==38)
                {
...

                    {
                        ...
                        {
                            var idScrollWrap=scrollWrap.attr("id"),hOption=-parseInt(scrollWrap.find("span").eq(0).outerHeight());
                            jQuery("#"+idScrollWrap)[0].scrollBy(hOption);
                        }
                        return false;
                    }
