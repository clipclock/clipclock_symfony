	...
	<?php include_component('user', 'navigationPerson', array('subject' => $user, 'active' => 'comments', 'current_user' => $current_user, 'user' => $user,
...
<div class="content-wrap-in">
	...
	<div class="long-col">
		...
		<ul id="container" class="stickers-list" style="position: relative;">
			<?php include_component('user', 'comments', array('user' => $user, 'pager' => $pager, 'current_user' => $current_user)) ?>
		</ul>
		</div>
		<script type="text/javascript">
			layoutAndScroll('<?php echo url_for('my_comments_page', array('nick' => $user->getNick(), 'page' => $pager->getNextPage())) ?>');
		</script>
	</div>
</div>
